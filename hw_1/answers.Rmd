...
3. Feeding behavior might also explain why certain species are distributed this way. Certain species of insects, fungi or plants may act as decomposers, attracting to areas where another animal has died. In this example, the high-density patches would not most likely not be consistent year to year.
...
first test showed nothing, second test showed stuff, so suggests that there are some sites that are doing much better than others and there merits analysis of those individual sites. 
...
grizzly_plot_2 <- ggplot(grizzly_mid, aes(x = Year, y = N)) +
  geom_line()+
  theme_minimal()+
  labs(x = "Year", y = "Bear Abundance", title = "Yearly Grizzly Bear population 1969 - 1978")+
  theme(text = element_text(family = "Times"))
...
```
For the last 10 years represented in the data set, there appears to be an increase in bear population, with 22 more bears in 1979 than in 1968. Using linear regression, this amounts to an on average incease of 3.15 bears per year. 
